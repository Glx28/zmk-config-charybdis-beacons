<#
Builds the left-half Charybdis firmware target in a ZMK Docker build image.
...
west update
west build -p -d /work/build/charybdis_left -s /work/zmk/app -b nice_nano_v2 -- -DSHIELD=charybdis_left -DZMK_CONFIG=/work/config -DZMK_EXTRA_MODULES=/work/zmk-pmw3610-driver -DZephyr_DIR=/work/zephyr/share/zephyr-package/cmake
cp /work/build/charybdis_left/zephyr/zmk.uf2 /work/firmware/charybdis_left.uf2
'@
...
$uf2 = Join-Path $buildFirmwareDir "charybdis_left.uf2"
if (-not (Test-Path -LiteralPath $uf2)) {
    throw "Docker build finished without producing expected UF2: $uf2"
}

$publicUf2 = Join-Path $publicFirmwareDir "charybdis_left.uf2"
...
Write-Host "Built left-half firmware:" -ForegroundColor Green
Write-Host $publicUf2
Write-Host "Flash manually by putting the left half in bootloader mode and copying this UF2 to the mounted drive." -ForegroundColor Yellow

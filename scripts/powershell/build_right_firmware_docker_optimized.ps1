...
OPTIMIZED VERSION - Uses narrow/shallow clone to avoid timeout on picolibc fetch.

Run from the project root:
  powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\powershell\build_right_firmware_docker_optimized.ps1
...
# Configure git for performance with large repos
...
# Use narrow update to avoid fetching all branches from huge repos like picolibc
west update --narrow --fetch-opt=--depth=1 || west update
